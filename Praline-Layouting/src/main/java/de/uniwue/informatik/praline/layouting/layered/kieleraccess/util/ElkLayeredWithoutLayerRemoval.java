/**
 * in praline: almost 1:1 copy with the goal not to remove the nodes from the layer immediately because we need to
 * save the mapping if nodes to layers (resp. layers to nodes) -- ideally without checking the y-coordinates since
 * the nodes might have different height.
 *
 * see comment ###---change---### in the text
 */
 ...
 *  {@link org.eclipse.elk.alg.layered.LayeredLayoutProvider#layout(ElkNode, IElkProgressMonitor)}
 *     instead.</li>
 * </ol>
 ...
 */
public final class ElkLayeredWithoutLayerRemoval {
     ...
     * ###---change---###
     *
     * both variables had to be extracted from a local variable to access them later
     */
    private List<LGraph> components;
    private IElkProgressMonitor theMonitor;

    public List<LGraph> getComponents() {
        return components;
    }
    ...
    public void doLayout(final LGraph lgraph, final IElkProgressMonitor monitor) {
        ...
        components = componentsProcessor.split(lgraph);
        ...
        the following was moved to finishLayouting(lGraph)

        To access List<LGraph> components there, I had to made it a class variable instead of a local variable
...
//        theMonitor.done();
     ...
     * {@link #runLayoutTestStep(TestExecutionState)} as often as there are layout processors.
     *
    ...
    /**
     * Performs the {@link #runLayoutTestUntil(Class, boolean, TestExecutionState)} methods with {@code inclusive} set
     * to {@code true}.
     *
     * @param phase the phase or processor to stop after
     * @param state the current test execution state
     * @see ElkLayeredWithoutLayerRemoval#runLayoutTestUntil(Class, boolean, TestExecutionState)
     */
    public void runLayoutTestUntil(final Class<? extends ILayoutProcessor<LGraph>> phase,
    ...
    private void layout(final LGraph lgraph, final IElkProgressMonitor monitor) {
        ...
        the following was extracted to an extra method, so that we can first save the mapping between
        layers and nodes, and then remove the nodes from the layers

        Only the last 3 lines regarding the monitor were kept here
     ...
     * ###---change---###
     *
     * New method to be called after having computed the layout.
     *
     * Mainly to remove the vertices from the layers last
     *
     * @param lgraph
     */
    public void finishLayouting(LGraph lgraph) {
        ...
        for (LGraph component : components) {
            for (Layer layer : component) {
                component.getLayerlessNodes().addAll(layer.getNodes());
                layer.getNodes().clear();
            }
            for (LNode node : component.getLayerlessNodes()) {
                node.setLayer(null);
            }
     ...
     * {@link ElkUtil#resizeNode(ElkNode, double, double, boolean, boolean)}.</p>
     *
